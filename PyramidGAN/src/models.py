...
class Transfer_Model(BaseModel):
    ...
    def process(self, input, target, stage):
        ...
        x_in = F.interpolate(input, (self.size_list[stage], self.size_list[stage]), mode='bilinear', align_corners=True)
        for xidx in range(0, stage + 1):
            x_tmp = F.interpolate(input, (self.size_list[xidx], self.size_list[xidx]), mode='bilinear',
                                  align_corners=True)
            x_in_list.append(x_tmp)
        x_out_list = self.generator(x_in_list)
        if x_in_list[-1].shape != x_out_list[-1].shape:
            x_out_list[-1] =  F.interpolate(target, (x_in_list[-1].size(-2), x_in_list[-1].size(-1)), mode='bilinear', align_corners=True)

        target = F.interpolate(target, (self.size_list[stage], self.size_list[stage]), mode='bilinear', align_corners=True)
    ...
    def forward(self,input):
        x_in = F.interpolate(input, (self.size_list[self.stage], self.size_list[self.stage]), mode='bilinear', align_corners=True)
        x_in_list = []
        for xidx in range(0, self.stage + 1):
            x_tmp = F.interpolate(input, (self.size_list[xidx], self.size_list[xidx]), mode='bilinear',
                                  align_corners=True)
            x_in_list.append(x_tmp)
